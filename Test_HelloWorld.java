
public class Test_HelloWorld {
	public static void main(String[] args) {
		System.out.println("Test de GitHub 25/01/18 - Hello World !");
		System.out.println("Dimitri a réussi a modifier le fichier");
		System.out.println("Dimitri a réussi a modifier le fichier via l'application GitHub Desktop");
		System.out.println("Yannis a réussi a modifier le fichier");
		System.out.println ("Eva a réussi a modifier le fichier");
		System.out.println ("Paul a réussi a modifier le fichier");
		
		while (true) {
			System.out.println("Simon est un crétin !");
		}

	}

}
